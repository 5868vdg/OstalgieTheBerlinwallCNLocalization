...
{
	// Token: 0x06000015 RID: 21 RVA: 0x00006548 File Offset: 0x00004748
	private void Awake()
	{
	...
	// Token: 0x06000016 RID: 22 RVA: 0x000065B4 File Offset: 0x000047B4
	public void Repaint()
	{
		...
		if (this.this_type == 1)
		{
			...
			base.GetComponent<OkoshkoScript>().text_en = "<color=yellow> 每 周:</color> 预 算 +0.1;\n 生 活 条 件 +0.1;";
			return;
		}
		if (this.this_type == 2)
		{
			base.GetComponent<OkoshkoScript>().text = "<color=yellow>Еженедельно:</color> Бюджет +0.2; Вестальгия +0.1;";
			base.GetComponent<OkoshkoScript>().text_en = "<color=yellow> 每 周:</color> 预 算 +0.2; 西 方 情 结 +0.1;";
			return;
		}
		if (this.this_type == 3)
		{
			...
			{
				base.GetComponent<OkoshkoScript>().text = "<color=yellow>Еженедельно:</color> Бюджет -0.3; Вестальгия -0.2;\nУровень жизни +0.2;";
				base.GetComponent<OkoshkoScript>().text_en = "<color=yellow> 每 周:</color> 预 算 -0.3; 西 方 情 结 -0.2;\n 生 活 条 件 +0.2;";
				return;
			}
			if ((this.global1.science[2] && this.global1.science[5] && this.global1.science[8]) || this.global1.science[9])
			{
				base.GetComponent<OkoshkoScript>().text = "<color=yellow>Еженедельно:</color> Бюджет -0.2; Вестальгия -0.2;\nУровень жизни +0.1;";
				base.GetComponent<OkoshkoScript>().text_en = "<color=yellow> 每 周:</color> 预 算 -0.2; 西 方 情 结 -0.2;\n 生 活 条 件 +0.1;";
				return;
			}
			base.GetComponent<OkoshkoScript>().text = "<color=yellow>Еженедельно:</color> Бюджет -0.2; Вестальгия -0.1;\nУровень жизни +0.1;\nРазвивает науку;";
			base.GetComponent<OkoshkoScript>().text_en = "<color=yellow> 每 周:</color> 预 算 -0.2; 西 方 情 结 -0.1;\n 生 活 条 件 +0.1;\n 发 展 科 学;";
		...
		else
		{
			...
			if (this.this_type == 5)
			{
				...
				base.GetComponent<OkoshkoScript>().text_en = "<color=yellow> 每 周:</color> 人 民 支 持 +0.2; 西 方 情 结 +0.2;";
				return;
			}
			if (this.this_type == 6)
			{
				base.GetComponent<OkoshkoScript>().text = "<color=yellow>Каждые 2 месяца:</color> Деньги +0.1;\nВестальгия +0.1;";
				base.GetComponent<OkoshkoScript>().text_en = "<color=yellow> 每 两 月:</color> 预 算 +0.1;\n 西 方 情 结 +0.1;";
				return;
			}
			if (this.this_type == 7)
			{
				base.GetComponent<OkoshkoScript>().text = "<color=red>Нельзя в столице, 1 на регион.</color>\n";
				base.GetComponent<OkoshkoScript>().text_en = "<color=red> 每 地 区 一 个 （ 除 首 都 ）</color>\n";
			...
			if (this.this_type == 10)
			{
				...
				base.GetComponent<OkoshkoScript>().text_en = "<color=yellow> 每 周:</color> 预 算 -0.2; 间 谍 网 络 +0.2;";
				return;
			}
			if (this.this_type == 11)
			{
				base.GetComponent<OkoshkoScript>().text = "<color=yellow>Еженедельно:</color> Бюджет -0.2; Поддержка народа +0.2;";
				base.GetComponent<OkoshkoScript>().text_en = "<color=yellow> 每 周:</color> 预 算 -0.2; 人 民 支 持 +0.2;";
				return;
			}
			if (this.this_type == 12)
			{
				...
				base.GetComponent<OkoshkoScript>().text_en = "<color=yellow> 每 周:</color> 苏 联 认 可 -0.1; 西 方 情 结 -0.2;\n 间 谍 网 络 +0.2;";
				return;
			}
			...
			if (this.this_type == 14)
			{
				...
				base.GetComponent<OkoshkoScript>().text_en = "<color=yellow> 每 周:</color> 党 内 团 结 +0.4; 主 权 +0.2;\n 北 约 不 满 +0.1; 苏 联 认 可 -0.1;";
				return;
			}
			if (this.this_type == 15)
			{
				base.GetComponent<OkoshkoScript>().text = "<color=yellow>Еженедельно:</color> Бюджет +0.3; Вестальгия +0.1;\nУровень жизни +0.2;";
				base.GetComponent<OkoshkoScript>().text_en = "<color=yellow> 每 周:</color> 预 算 +0.3; 西 方 情 结 +0.1.;\n 生 活 条 件 +0.2;";
				return;
			}
			if (this.this_type == 17)
			{
				if (this.global1.data[14] < 3)
				{
					...
					base.GetComponent<OkoshkoScript>().text_en = " 每 月: 人 民 支 持 +0.1;";
				}
				...
				else
				{
					...
					base.GetComponent<OkoshkoScript>().text_en = " 无;";
				}
	...
	// Token: 0x06000017 RID: 23 RVA: 0x00006C88 File Offset: 0x00004E88
	private void OnMouseDown()
	{
		if (this.buildmanager1.now_region != 2 && (this.global1.data[0] == 49 || this.global1.data[0] == 50 || this.global1.data[0] == 51))
		{
			...
			{
				if ((this.yug1.gameState.yugregions[this.buildmanager1.yugreg[1]].owner != 7 || this.yug1.gameState.yugregions[7].owner != 7 || this.global1.data[156] != 2 || this.global1.data[0] != 49) && !this.buildmanager1.yugown[1])
				{
					this.Vyzov.text = this.yug1.science_text[65];
					return;
				}
			}
			else if (this.yug1.gameState.yugregions[this.buildmanager1.yugreg[2]].level + 5 > this.buildmanager1.selected_thing && (this.yug1.gameState.yugregions[this.buildmanager1.yugreg[2]].owner != 2 || this.yug1.gameState.yugregions[2].owner != 2 || this.yug1.gameState.yugcountries[2].is_exist || !this.global1.event_done[274] || this.global1.data[0] != 51) && (this.yug1.gameState.yugregions[this.buildmanager1.yugreg[2]].owner != 2 || this.yug1.gameState.yugregions[2].owner != 2 || !this.yug1.gameState.yugcountries[2].is_exist || this.global1.data[150] != 1 || this.global1.data[0] != 49) && !this.buildmanager1.yugown[2])
			{
				this.Vyzov.text = this.yug1.science_text[65];
				return;
			}
			...
			if (this.global1.data[8] >= 25)
			{
				...
				else if (this.this_type == 4)
				{
					...
					if ((this.global1.regions[this.buildmanager1.now_region].city_level % 2 == 1 && num2 >= (this.global1.regions[this.buildmanager1.now_region].city_level + 1) / 2) || (this.global1.regions[this.buildmanager1.now_region].city_level % 2 == 0 && num2 >= this.global1.regions[this.buildmanager1.now_region].city_level / 2))
					{
						if (PlayerPrefs.GetInt("language") == 0)
						{
							this.Vyzov.text = " 每 二 城 市 等 级 一 个";
						}
						else
						{
				...
				else if (this.this_type == 17)
				{
					...
					if (!flag2)
					{
						for (int m = 0; m < 15; m++)
						{
							if ((this.global1.data[0] < 49 || this.global1.data[0] > 51) && this.global1.regions[this.buildmanager1.now_region].buildings[m].type == 17 && this.global1.regions[this.buildmanager1.now_region].buildings[m].is_builded && this.global1.regions[this.buildmanager1.now_region].buildings[m].is_working)
							{
								flag2 = true;
								break;
							}
	...
	// Token: 0x06000018 RID: 24 RVA: 0x00007820 File Offset: 0x00005A20
